...
fn lighthouse_verdict(state: &BeaconState<MainnetEthSpec>, att: &Attestation<MainnetEthSpec>) -> bool {
...
fn grandine_verdict_attestation(state_path: &str, att_bytes: &[u8]) -> Option<bool> {
    ...
    if let Ok(gatt_e) = gtypes::electra::containers::Attestation::<GMainnet>::from_ssz(&*GCONFIG, att_bytes) {
        ...
        return Some(gtrans::electra::validate_attestation_with_verifier(&*GCONFIG, &*GPUBKEY, &gstate, &gatt_e, GNullVerifier).is_ok());
    }
...
fuzz_target!(|data: &[u8]| {
    ...
    // Verdicts
    let lh_ok = lighthouse_verdict(&CTX.state, &att);
    if let Some(gr_ok) = grandine_verdict_attestation(&CTX.path, data) {
        if lh_ok != gr_ok {
