...
extern crate types;
use types::{BeaconState, ChainSpec, EthSpec, MainnetEthSpec, SignedVoluntaryExit};
...
fn lighthouse_verdict(state:&BeaconState<MainnetEthSpec>, x:&SignedVoluntaryExit)->bool{let spec:ChainSpec=MainnetEthSpec::default_spec();let mut s=state.clone();process_exits(&mut s,&[x.clone()],VerifySignatures::False,&spec).is_ok()}
lazy_static!{ static ref GCONFIG:GConfig=GConfig::mainnet(); static ref GPUBKEY:GPubkeyCache=GPubkeyCache::default(); }
fn grandine_verdict(bytes:&[u8], state_path:&str)->Option<bool>{ let x=gtypes::phase0::containers::SignedVoluntaryExit::from_ssz(&*GCONFIG,bytes).ok()?; let st=std::fs::read(state_path).ok()?; let gstate=gtypes::combined::BeaconState::<GMainnet>::from_ssz(&*GCONFIG,&st).ok()?; Some(gunphased::validate_voluntary_exit(&*GCONFIG,&*GPUBKEY,&gstate,&x).is_ok()) }

fn run_grandine_oracle(bytes:&[u8], state_path:&str)->io::Result<Option<bool>>{let cmd=match std::env::var("DIFF_ORACLE_CMD"){Ok(c) if !c.trim().is_empty()=>c,_=>return Ok(None)};let sh=format!("{} --type voluntary_exit --state {}",cmd,shell_escape::escape(state_path));let mut ch=Command::new("/bin/sh").arg("-lc").arg(sh).stdin(Stdio::piped()).stdout(Stdio::piped()).stderr(Stdio::null()).spawn()?;if let Some(mut i)=ch.stdin.take(){let _=i.write_all(bytes);}let out=ch.wait_with_output()?;if !out.status.success(){return Ok(None);}let s=String::from_utf8_lossy(&out.stdout).to_string();let s=s.trim().to_ascii_lowercase();Ok(if s.contains("ok")||s=="1"||s=="true"{Some(true)}else if s.contains("err")||s=="0"||s=="false"{Some(false)}else{None})}
...
fuzz_target!(|data:&[u8]|{let x:SignedVoluntaryExit=match SignedVoluntaryExit::from_ssz_bytes(data){Ok(v)=>v,Err(_)=>return};let lh_ok=lighthouse_verdict(&CTX.state,&x); if let Some(gr_ok)=grandine_verdict(data,&CTX.path){ if lh_ok!=gr_ok{panic!("DIFF MISMATCH: lighthouse={}, grandine={}",lh_ok,gr_ok);} }});
