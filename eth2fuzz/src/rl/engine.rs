...
impl RLEngine {
    ...
    pub fn initialize(&mut self, filter: &str) -> Result<(), Error> {
        ...
        if let Some(id) = &self.run_id {
            ...
            let init_summary = summarize_corpora(&ws)?;
    ...
    fn run_one_segment(
        ...
        let source_dir: std::path::PathBuf = if let Some(label) = &arm.bin_label {
            if let Some(tmap) = self.bins.get(&arm.target_name) {
                if let Some(dir) = tmap.get(label) { std::path::PathBuf::from(dir) } else { ws.join("corpora").join(&corpora_label) }
            } else { ws.join("corpora").join(&corpora_label) }
        } else { ws.join("corpora").join(&corpora_label) };
        // prepare batch dir
        let batch_dir = ws.join("rl_batch").join(&arm.target_name);
        ...
        // sample up to 64 files
        let _ = sample_batch(&source_dir, &batch_dir, 64);
        ...
        if let (Some(paths), Some(st)) = (&self.run_paths, self.run_stats.as_mut()) {
            ...
            st.segments.push(SegmentRecord { index: self.seg_index, target: arm.target_name.clone(), bin: arm.bin_label.clone(), batch_count, reward: base + time_bonus + dense });
